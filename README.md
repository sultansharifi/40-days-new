...
- یک گزارش یعنی یک متن کوتاه — بدون فرم چندمرحله‌ای، بدون فیلدهای اضافه
- داشبورد یکپارچه: کارت‌های آماری و لیست کامل گزارش‌ها با اسکرول بی‌نهایت،
  جستجو، و خروجی Excel/PDF
- روی هر گزارش یک تیک «انجام شده؟» — تا زده نشود، گزارش «در انتظار» می‌ماند
- منوی «گزارش جدید»: یک باکس متن + دکمه «ایجاد گزارش»؛ گزارش‌های قبلی هم
  به‌صورت دکمه نشان داده می‌شوند — با یک کلیک، همان متن دوباره ثبت می‌شود
...
- **Backend:** Supabase (PostgreSQL، توابع دیتابیسی برای ذخیره اتمیک
  گزارش) — بدون Auth، دسترسی به‌صورت باز (RLS فعال ولی همه اجازه دارند)
- دیتا: TanStack Query — تقویم: شمسی/جلالی
...
```
src/
  components/ui/       کامپوننت‌های پایه (دکمه، کارت، دیالوگ، ...)
  components/layout/   سایدبار (داشبورد + گزارش جدید)، هدر
  components/reports/  فرم ساده گزارش، لیست گزارشات، تیک «انجام شده»
